...
#ifdef USE_WIRE
    if (es == NULL || es->wire == NULL) {
        return ESP_ERR_INVALID_ARG;
    }
    es->wire->beginTransmission(es->dev_addr);
    es->wire->write(write_buf, sizeof(write_buf));
    return (es->wire->endTransmission(true) == 0) ? ESP_OK : ESP_FAIL;
#else
...
static inline esp_err_t es8311_read_reg(es8311_handle_t dev, uint8_t reg_addr, uint8_t *reg_value)
{
    es8311_dev_t *es = (es8311_dev_t *) dev;
#ifdef USE_WIRE
    if (es == NULL || es->wire == NULL || reg_value == NULL) {
        return ESP_ERR_INVALID_ARG;
    }
    es->wire->beginTransmission(es->dev_addr);
    es->wire->write(reg_addr);
    if (es->wire->endTransmission(false) != 0) {
        return ESP_FAIL;
    }
    if (es->wire->requestFrom((int)es->dev_addr, 1) != 1) {
        return ESP_FAIL;
    }
    *reg_value = es->wire->read();
    return ESP_OK;
#else
...
es8311_handle_t es8311_create(const es8311_i2c_t bus, const uint16_t dev_addr)
{
...
{
#ifdef USE_WIRE
    ESP_LOGE(TAG, "es8311_codec_init() example uses legacy I2C driver. Define USE_WIRE but call es8311_create(&Wire, addr) from your app instead.");
