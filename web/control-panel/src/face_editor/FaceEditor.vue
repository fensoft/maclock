...
async function saveProject() {
  ...
  if (window.showDirectoryPicker) {
    try { const root = await window.showDirectoryPicker({ mode: "readwrite" }); const folder = await root.getDirectoryHandle(base, { create: true }); for (const item of project.value.objects.filter((item) => item.type === "image")) await writePng(folder, item); const file = await folder.getFileHandle(`${base}.json`, { create: true }); const writer = await file.createWritable(); await writer.write(projectJson()); await writer.close(); status.value = `Saved ${base}/`; return; } catch (error) { if (error.name !== "AbortError") status.value = "Folder save unavailable; downloaded JSON instead."; else return; }
  }
